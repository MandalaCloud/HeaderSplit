...
def bundled_label2origin(blocks, labels, N):
    '''
    预处理数据聚类后的结果 -> 预处理前的格式
...
def load_multi(cousage, semantic):
    
    truefeatures = np.concatenate((cousage, semantic), axis=1)
...
def load_data(cousage, semantic):
    """Load data."""
    labels, adjs, feature, adjs_labels, feature_label, num_graph = load_multi(cousage, semantic)
    features = []
    feature_labels = []
...
def calculate_modularity(y_pred, adj):
