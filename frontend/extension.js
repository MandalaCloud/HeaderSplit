...
async function executePythonScript(pythonFilePath, projectDir, filePath, parts, name, useGPT = "false", key = "", URL = "", model = "") {
    let commandline;
    if (name === "Construct") commandline = `python ${pythonFilePath} ${projectDir} ${filePath} ${parts} ${useGPT} ${key} ${URL} ${model}`;
    else commandline = `python ${pythonFilePath} ${projectDir} ${filePath} ${parts}`;
...
function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('extension.file-decomposing', async (uri) => {
        ...
        while (!start_refactor) {
            ...
            const community_index = path.join(context.extensionPath, 'intermediate_data', `plan_overview_${path.basename(uri.path)}_${parts}.json`);
        ...
        try {
            ...
            await executePythonScript(pythonFilePath, project_dir, filePath, parts, 'Refactor');
